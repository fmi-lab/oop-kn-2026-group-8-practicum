...
Забелязваме, че в конструктора на класа заделяме динамична памет, но никога не я освобождаваме. Предния път вече разгледахме този проблем и знаем, че той се разрешава с помощта на деструктора на класа:
...
Случаите, в които реализираме само част от тези 3 специални метода са изключително редки и най-вероятно в курса ще видим само един такъв пример. По принцип има още 2 специални метода, които трябва да реализираме във втория случай, но за тях ще си говорим по-късно в курса. Когато сме реализирали и трите специални метода, казваме, че имаме голяма четворка или `rule-of-3` (името голяма четворка идва от това, че към тези методи се брои и обикновения конструктор).
...
Пренапишете задача 02 от предния път, така че да е спазено правилото за голямата четворка.